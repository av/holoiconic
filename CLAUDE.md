...
Nodes receive `ctx` (5 primitives + `ctx.self`) and `args` (optional caller-provided object). Nodes cannot use `import` — they run in AsyncFunction scope with access to Bun globals, `process`, and `console`.
...
- `set` node is for single-valued predicates: retracts old, asserts new, serialized per key
